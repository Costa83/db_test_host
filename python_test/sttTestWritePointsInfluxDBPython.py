...
def write_point(points, ip, port) :

    database_name="test_influxdb_python"
    start_time = time.time()
    client = InfluxDBClient(ip, port)
    ...
    for i in range(number_of_points):
        ...
        data.append("{measurement},source={source},type={type},unit={unit},idtag={idtag} id={id},value={value} {timestamp}"
...
if __name__ =="__main__":
    args=parse_args()
    write_point(args.points, args.ip, args.port)
